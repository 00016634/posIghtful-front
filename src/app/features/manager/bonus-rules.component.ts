      ...
      <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1 class="text-2xl font-semibold">Bonus Rules</h1>
              <p class="text-sm text-muted-foreground">Configure bonus calculation rules</p>
            </div>
          </div>
          <div class="flex gap-2">
            <ui-button variant="outline" (click)="router.navigateByUrl('/manager/bonus-management/attribution-window')">
            ...
            <ui-button (click)="router.navigateByUrl('/manager/bonus-management/rules/new')">
        ...
        <ui-card>
          <ui-card-header>
            <ui-card-title>Active & Inactive Rules</ui-card-title>
          </ui-card-header>
          <ui-card-content>
            <ui-table>
              <ui-table-header>
                <ui-table-row>
                  <ui-table-head>Rule Name</ui-table-head>
                  <ui-table-head>Dimension</ui-table-head>
                  <ui-table-head>Condition</ui-table-head>
                  <ui-table-head>Payout</ui-table-head>
                  <ui-table-head>Priority</ui-table-head>
              ...
              <ui-table-body>
                @for (rule of rules(); track rule.id) {
                  <ui-table-row>
                    ...
                    <ui-table-cell>
                      <ui-badge variant="outline">{{ rule.dimension }}</ui-badge>
                    </ui-table-cell>
                    <ui-table-cell className="text-sm">
                      <span class="font-medium">{{ rule.operator }}</span>
                      <span class="text-muted-foreground ml-1">{{ rule.value }}</span>
                    </ui-table-cell>
                    <ui-table-cell className="font-medium text-green-600">{{ rule.payoutValue }}</ui-table-cell>
                    <ui-table-cell>{{ rule.priority }}</ui-table-cell>
                    <ui-table-cell>
                      <button (click)="toggleActive(rule.id)">
                        <span [class]="'inline-flex items-center rounded-md px-2.5 py-0.5 text-xs font-semibold ' + (rule.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800')">
                    ...
                    <ui-table-cell className="text-right">
                      <div class="flex justify-end gap-2">
                        <ui-button variant="ghost" size="icon" (click)="router.navigateByUrl('/manager/bonus-management/rules/' + rule.id)">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>
                        </ui-button>
                        <ui-button variant="ghost" size="icon" (click)="deleteRule(rule.id)">
                  ...
                  <ui-table-row>
                    <ui-table-cell className="text-center py-8 text-muted-foreground" colSpan="7">
                      No rules configured
                    </ui-table-cell>
                  </ui-table-row>
                }
